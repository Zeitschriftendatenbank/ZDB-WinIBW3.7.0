...
2019-03-25 gr Neues Skript ORCiD hinzugefügt
*/
...
function relaCodeVergabe() {

//2018-04-12  gr  Aus Abwärtskompabilitätsgründen kann das Skript sowohl über rela... als auch über rel... aufgerufen werden
...
/******************************************************************************************************************************************************
Die Funktion übernimmt aus einem GND-Satz in Vollanzeige oder Korrekturmodus automatisch die Inhalte aus dem Feld 100, öffnet den Internetbrowser und führt eine Suche nach dieser Person auf der ORCiD-Webseite durch. Falls bereits eine ORCID im GND-Datensatz vorhanden ist, wird der entsprechende ORCID-Datensatz aufgerufen


2018-07-??  ba  Neuerstellung
2019-02-07  gr  Anpassungen: URL wird jetzt so gebildet, dass eine Phrasensuche nach dem Namen stattfindet (voher: Vorname *oder* Nachname); Umlaute werden URL-encodiert; falls ein Feld 024 mit orcid vorhanden ist, wird diese gesucht und nicht der Name
2019-02-15  gr  Korrektur: ORCID wird jetzt auch korrekt ermittelt, wenn $v folgt
2019-03-08  gr  RegEx für die Prüfung einer ORCiD korrigiert: "X" jetzt auch erlaubt
******************************************************************************************************************************************************/

	// Name, Vorname auslesen
	var strTag005;
	var strTag100 = "";
	var strTag024 = "";
	var strTag024orc = "";
	...
	var boolEdit;
	var i = 0;
	var boxtit = "ORCiD-Aufruf";
	var strScreen = application.activeWindow.getVariable("scr");
	//Ermitteln der relevanten Feldinhalte, je nachdem ob Datensatz in Korrektur-/Erfassungs- oder Vollanzeige ist 
	if (strScreen == "8A") {
		strTag005 = application.activeWindow.findTagContent("005",0, false);
		strTag100 = application.activeWindow.findTagContent("100",0, false);
		//Leerzeichen am Anfang und Schmutzzeichen am Ende entfernen
		strTag100 = strTag100.substring(1,strTag100.length-1);
		while (strTag024 = application.activeWindow.findTagContent("024",i,false)) {
			if (strTag024.indexOf("orcid") > -1) {
			strTag024orc = strTag024;
			break;
			}
		i++;
		}
	} else {
		if ((strScreen == "MI") || (strScreen == "II" )) { 
		strTag005 = application.activeWindow.title.findTag("005",0,false,true,false);
		strTag100 = application.activeWindow.title.findTag("100",0,false,true,false);
			while (strTag024 = application.activeWindow.title.findTag("024",i,false,true,false)) {
				if (strTag024.indexOf("orcid") > -1) {
				strTag024orc = strTag024;
				break;
				}
		...
		application.messageBox(boxtit,"Der Normdatensatz muss sich in der Vollanzeige oder im Edit-Modus befinden!", "alert-icon");
		return;
		}
	}
	
		
		//Ausschneiden der ORCID
		if (strTag024orc != "") {
			if (strTag024orc = strTag024.match(/\d{4}-\d{4}-\d{4}-\d{3}[\dX]/)) {
			} else {
			__gndError(boxtit,"ORCiD '" + strTag024 + "' ist nicht korrekt erfasst!");
			return;
			}
		}
				
	
	if (strTag005.match(/T[np]./)) {
		// wenn erfolgreich, Browser starten und Trefferliste bei orcid.org anzeigen
		
		//wenn ORCID vorhanden ist, wird diese aufgerufen, sonst Suche nach dem Namen
		if (strTag024orc != "") {
		strRequest = "https://orcid.org/" + strTag024orc;
		} else if (strTag100 != "") {
		posFirstName = strTag100.indexOf(",");
		posNobleTitle = strTag100.indexOf("$c");			
			if ((posFirstName > 0) && (posNobleTitle > 0)) {
				strSurname = strTag100.substring(0,posFirstName);
				strFirstName = strTag100.substring(posFirstName+2,posNobleTitle);
				strNobleTitle = strTag100.substring(posNobleTitle+2,strTag100.length);
			} else {
				if (posFirstName > 0) { 
				strSurname = strTag100.substring(0,posFirstName);
				strFirstName = strTag100.substring(posFirstName+2,strTag100.length);
				} else {
				strSurname = strTag100.substring(0,strTag100.length);
				}
			}
		strRequest = "https://orcid.org/orcid-search/quick-search/?searchQuery=" + encodeURIComponent('"' + strFirstName + ' ' + strSurname + '"');
		}
		
		application.shellExecute(strRequest,5, "open", "");
	} else {
		application.messageBox(boxtit,"Für diese Funktion muss ein Personennormdatensatz in der Vollanzeige aufgerufen werden!","");
	}
