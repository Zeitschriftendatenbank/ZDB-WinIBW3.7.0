...
function zdb_KennungWechseln(){
    ...
    if (wert.indexOf('ZDB-Hauptbestand') >= 0 || wert.indexOf('ZDB-Uebungsbestand') >= 0) {
        ...
        application.messageBox('KennungWechseln', "Die Funktion \"KennungWechseln\" kann nur im ZDB-Hauptbestand oder ZDB-�bungsbestand aufgerufen werden", 'alert-icon');
        return;
    }
}
...
function zdb_ExemplarErfassen(){
    ...
    __zdbExemplarErfassen(exNum, content, goToLine);
...
function __zdbExemplarNummer(){
    ...
    //Schleife von 1 bis 99, da max. 99 Exemplare pro Bibliothek erfasst werden k�nnen
    for (var i = 1; i <= 99; i++) {
...
function zdb_HoleIDN(){
    // Wurde vorab eine Suche mit dem Skript "Automatische Suchbox" ausgef�hrt?
    if (typeof anfangsfenster == 'undefined') {
        application.messageBox('HoleIDN', "Vor Aufruf des Skriptes \"HoleIDN\" muss zun�chst eine automatische Suche mit Hilfe des Skriptes \"AutomatischeSuchBox\" gestartet werden.", 'alert-icon');
    } else {
        ...
        try {
            // Zur�ck zum Anfangsfenster gehen
            application.activateWindow(anfangsfenster);
            // IDN einf�gen
            application.activeWindow.title.insertText('!' + idn + '!');
            // Trefferliste schlie�en
            application.closeWindow(fenster);
        } catch(e) {
            application.messageBox('HoleIDN', 'Das Bearbeitungsfenster, in welches die IDN eingef�gt werden soll, ist nicht mehr ge�ffnet.', 'alert-icon');
        }
    }
    return true;
}
...
function __zdbTiteldatenKopie(){
    ...
    if(!_rec['002E']) { // 0503 Datentr�gertyp
        __zdbDatentraeger();
...
/**
* Der Inhalt von 0503 ist abh�ngig von 0500 und 0502
* 
...
/**
* Der Inhalt von 0502 ist abh�ngig von 0500 und 0501
* 
* Die Funktion erwartet ein globales Objekt _rec
...
function zdb_Datensatzkopie() {
    ...
    //Pers�nliche Einstellung des Titelkopie-Pfades ermitteln
    ...
    //Wiederherstellen des urspr�nglichen Pfades der Titelkopie-Datei:
...
function zdb_Digitalisierung() {
    ...
    // Pr�fen, ob Titeldatensatz mit bibliographischer Gattung "A" aufgerufen, bei "T" oder "O" Fehlermeldung ausgeben
    var matCode = application.activeWindow.materialCode.charAt(0);
    if(matCode == 'T' || matCode == 'O') {
        application.messageBox('Digitalisierung', "Die Funktion kann nur f�r Titels�tze des Satztyps \"A\" verwendet werden.", 'alert-icon');
        return false;
    }
...
function zdb_Parallelausgabe(){
    ...
    // Kategorie 4234: anlegen und mit Text "4243 Erscheint auch als$nDruckausgabe![...IDN...]!" bef�llen
    application.activeWindow.title.endOfBuffer(false);
    application.activeWindow.title.insertText("\n4243 Erscheint auch als$nDruck-Ausgabe!" + idn + "!\n");

    // Kategorie 4213: individuell gef�llt oder leer ausgeben
    application.activeWindow.title.endOfBuffer(false);
    application.activeWindow.title.insertText('4201 Gesehen am ++');
    application.activeWindow.title.charLeft(1,false);
...
function __zdbOnlineRessource(copyFile,showComment,add0600,digi){
    ...
    // Kategorie 0500: Bibliographische Gattung/Status �ndern
    ...
    // Kategorie 2010,4215,4225 �ndern
    var content,y;
    var fieldmap = {
        '2010': '2013 |p|',
        '4215': '4201 ',
    ...
    // neues Feld f�r Sekund�rk�perschaft 312X -> 311X
    content = '';
    y = 0;
...
function __zdbTitelAnpassen()
{
    ...
    if(__zdbCheckSF('021A','e')) // K�rperschaftserg�nzungen vhd.
    {
...
function __zdbFeld424XGet()
{
    ...
    // Verkn�pfungsfelder einsammeln und auf verbale Form �ndern
    var _felder424X = {
        '039B' : {p:'4241',c:[]},
        '039C' : {p:'4242',c:[]},
        '039D' : {p:'4243',c:[]},
    ...
    for(var f in _felder424X) //  f = 039X
    {
        if(_rec[f]) // Feld 039X vorhanden
        {
            for(var e in _rec[f]) // Wiederholungen
            {
                ...
                {
                    if('039E' != f || rda ) // kein Vortext f�r 4244 ohne rda
                    {
                        code += _rec[f][e]['a'][0];
                    }
                } 
...
function __zdbFeld424XSet(_felder424X)
{
    var _lang = {
        ...
        'Fr':'Parallele Sprachausgabe$nfranz�sisch',
...
function __zdbDruckausgabe(dppn){
    ...
    if (application.activeWindow.status != 'OK') {
        return __zdbError('Die �ber 4243 verlinkte Druckausgabe existiert nicht.');
    }
...
//	DocType = 1. Zeichen im Feld 0500
    ...
    if (arr == null) {
        ...
        if (application.activeWindow.status != 'OK') {
            __zdbMSG('Sie sind nicht berechtigt, den Datensatz zu �ndern.');
            return false;
        }
        ...
        //	Korrektur ausgef�hrt, dann ist der Titel im diagn. Format
        ...
        if (application.activeWindow.getVariable('scr') != '8A') {
            __zdbMsg('Die Korrektur des Titel ist fehlgeschlagen. Bitte holen'
                    + 'Sie dies direkt �ber die WinIBW nach.');
            return false;
        }
    } else {
        application.messageBox('Test','Die Verkn�pfung zur Internetausgabe im Feld 4243 ist schon vorhanden.', 'alert-icon');
    }

//---Feld '2010' , zur�ckgeben
...
function zdb_EZB_BibID(){
    //Anwender k�nnen BibID pr�fen und ggf. korrigieren
...
function zdb_EZB(){
    ...
    if(!bibid)
    {
        return __zdbError('Sie m�ssen ein g�ltige EZB-bibid angeben.');
    }
...
//---Feld '4000' , Inhalt nach title
    ...
    //---Feld '4005' , Inhalt an title anh�ngen
    if(_rec['021C'])
    {
        var unterreihe_bez = '',
            unterreihe_tit = '';
...
//---Druckausgabe: reziproke Verkn�pfung und Druck-ISSN
    ...
    // pissn �ber Verkn�pfung ermitteln
    if(_rec['039D'])
    {
    ...
    if(dppn)
    {
        ...
        if (!__zdbYesNo('Eine reziproke Verkn�pfung ist nicht m�glich. M�chten Sie trotzdem fortfahren?')) {
            return false;
        }
    }
...
//	4 bedeutet ja und nein; 6=ja 7=nein
    if (__zdbYesNo (
            ...
            + "und kontrollieren die �bereinstimmung Ihrer Aufnahme mit dem\n"
...
function __zdbCheckKurztitelAuswahl() {
    ...
    if (__zdbYesNo("Sie haben das Skript aus der Kurztitelliste aufgerufen.\n"
                + "Zur Sicherheit:\n\n"
                + "Ist dies der gew�nschte Datensatz?"))		return true;
    //application.activeWindow.simulateIBWKey('FE');
    return false;
}
...
*
* weitere Tests:
* --Abvz--: Adre�- und Gesch�ftshandbuch f�r den k[�niglich] b[ayerischen] Markt Berchtesgaden und Berchtesgadener-Land
* 
* --Abvz--International Legal Center$xAllgemeine Unterteilung$gNew York, NY$BVerfasser: Adre�- und Gesch�ftshandbuch f�r den k[�niglich] b[ayerischen] Markt Berchtesgaden und Berchtesgadener-Land
* 
...
* Bsp.:
* 039E �bf�aFortsetzung von�9942987667�8--Cbvz--Deutsche Zentralb�cherei f�r Blinde zu Leipzig: DZB-Nachrichten
* wird zu
* {
*   "039E":
*   {
...
*      "a": ["Fortsetzung von"],
*      "9": ["942987667"],
*      "8": ["--Cbvz--Deutsche Zentralb�cherei f�r Blinde zu Leipzig: DZB-Nachrichten"]
*   }
* }
*
...
function isil_online() {
    ...
    if('Tw' != application.activeWindow.getVariable('P3VMC')) {
        return __zdbError('Die Funktion kann nur in Verbindung mit Tw-S�tzen genutzt werden.');
    }
