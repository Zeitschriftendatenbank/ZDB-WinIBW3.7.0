...
try{
	...
	var datensatzNr = 0;
	var textKategorie;
	var meldungBedingung1 = "";
	var strBedingung2Kat = document.getElementById("idBedingung2Kat").value;
	var strBedingung2Inhalt = document.getElementById("idBedingung2Inhalt").value;
	var meldungBedingung2 = "";
	strfehlerMeldungen = "";
	...
	if(bcaseSensitive = document.getElementById("idCheckboxCase").checked) {
        caseFlag = '';
    }
...
try{
	...
	for (datensatzNr = 1; datensatzNr <= lSetsize; datensatzNr++){
		application.activeWindow.command("\\too " + datensatzNr, false);
		//welche Ebene darf bearbeitet werden?
		if (testEbene(loescheKat) == "2") {bearbeiteEbene2("loeschen");}
		else {bearbeiteEbene0und1("loeschen");}
		document.getElementById("idSchlussmeldung1").value = "Datensatz Nr. " + datensatzNr + " von " + lSetsize + " wird bearbeitet."
	}
	application.activeWindow.command("\\too k", false);
	document.getElementById("idSchlussmeldung1").value = "In " + lBearbeitet + " " + strSatzart + " wurde der Text ersetzt. ";
...
function weitereBedingungen(){
	...
	if (zeigeWeitereBedingungen == false){
		...
		if(strBedingung1 != "" || strBedingung2Kat != "" || strBedingung2Inhalt != ""){
			...
			//return;
			var prompter = utility.newPrompter();
			var antwort = prompter.confirmEx("Suche / Ersetze", "Erweiterte Bedingungen löschen?", "Ja", "Nein", "", "", "")
			if (antwort == 0) {
				//Felder löschen:
...
function fehlerDateiAnlegen()
{
	...
	var theRelativePath = "listen\\" + strDatumHeute + "_Fehler.txt";
	//Datei wird angelegt:
