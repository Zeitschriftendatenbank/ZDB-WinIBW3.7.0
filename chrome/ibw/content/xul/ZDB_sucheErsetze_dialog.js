...
function onLoad() {
    ...
    switch (strVerbund) {
        case "K10plus":
            ...
            //      strKatExe = "43[0-9][0-9]|480[12347]|4880|67[0-9xX][0-9xX]|68[0-9xX][0-9xX]|E[0-9xX][0-9xX][0-9xX]|71[0-9][0-9]|712[0123]|713[39]|7200|73[0-9][0-9]|7800|7901|8[0-8][0-9][0-9]";
            //01.2020: aktualisiert anhand der K10plus Format-Dokumentation:
            strKatLok = "208[0-9]|3433|348[0-9]|354[0-9]|471[0-9]|476[34]|4790|60[0-9xX][0-9xX]|61[0-9]|6110|65[0-9xX][0-9xX]|9000";
            strKatExe = "48[0-9][0-9]|6600|67[0-9xX][0-9xX]|68[0-9xX][0-9xX]|E[0-9xX][0-9xX][0-9xX]|71[0-4][0-9]|7200|73[0-9][0-9]|781[0-3]|8[0-6][0-9][0-9]";
            break;
        case "DNB":
            hinweisVZG = "Sie haben Felder der bibliographischen Ebene (Titelebene) ausgewählt. " +
            ...
            strELN = "7777|8007|8009|9001|9006|9002|6666|4065";
            strUser = "6001|6005|6199|6099|6004|6207|6257";
            strKatLok = "2080|348[01]|354[0-9]|471[056]|476[34]|60[0-9xX][0-9xX]|6100|65[0-9xX][0-9xX]";
            strKatExe = "E\\d\\d\\d|480[012]|4820|4822|6700|70[0-9xX][0-9xX]|710[0-9]|7120|713[0345678]|714[0-9]|715[09]|7[89]00|8001|803[12345]|8[12]00|844[89]|846[567]|8510|859[45678]";
            break;
        default:
    ...
    if (loeschenSichtbar) {
        ...
        document.getElementById("tabLoeschen").label = "Löschen von Felder";
...
function testEbene(kategorie) {
    ...
    //Voreinstellung:
    strEbene = "0";
    var oRegExpKatLok = new RegExp(strKatLok),//Felder der Lokalebene:
        oRegExpKatExe = new RegExp(strKatExe);//Felder der Exemplarebene:
...
//----------------------------------------------------------------
// Tab 1: Ersetzen von Inhalten in Felder
//----------------------------------------------------------------
...
function bearbeiteSetErsetzen() {
    ...
    try {
        ...
        if (Kat1.length < 3 || Kat1.length > 4) {
            alert("Bitte tragen Sie in beiden Feldern entweder 3- oder 4-stellige Felder ein!");
            return;
        }
        ...
        if (document.getElementById("idBedingung1").value != "") {
            meldungBedingung1 = "\nWenn in derselben Feld dieser Text vorkommt: " + document.getElementById("idBedingung1").value;
        }
        ...
        if (strBedingung2Kat != "" && strBedingung2Inhalt != "") {
            meldungBedingung2 = "\nWenn auch Feld " + strBedingung2Kat + " mit diesem Text vorkommt: " + strBedingung2Inhalt;
        }
...
function bearbeiteSetZufuegen() {
    try {
        ...
        if (wennKat.length != dannKat.length) {
            alert("Bitte tragen Sie in beiden Feldern entweder 3- bzw. 4-stellige Felder ein!");
            return;
        }
        ...
        //Kontrolle der Datensatzebene (hier nur einzufügende Feld):
        ...
        lSetsize = application.activeWindow.getVariable("P3GSZ");
        antwort = prompter.confirmEx("Ergänzen", "Suche Feld " + wennKat + " mit Text '" + wennText + "'" +
            "\nErgänze Feld " + dannKat + " mit Text '" + dannText + "'" +
            "\n\nWollen Sie jetzt alle " + lSetsize + " Datensätze bearbeiten?", "Ja", "Nein", "", "", "");
        //wenn Antwort 1 = nein:
        if (antwort == 1) {
            return;
...
function bearbeiteSetLoeschen() {
    try {
        ...
        if (isNaN(loescheKat)) {
            alert("Bitte geben Sie im Feld 'Lösche Feld' nur Ziffern ein.");
            return;
        }
        ...
        lSetsize = application.activeWindow.getVariable("P3GSZ");
        antwort = prompter.confirmEx("Löschen", "Suche und lösche Feld " + loescheKat + " mit Text '" + strSuche + "'" +
            "\n\nWollen Sie jetzt alle " + lSetsize + " Datensätze bearbeiten?", "Ja", "Nein", "", "", "");
        //wenn Antwort 1 = nein:
        if (antwort == 1) {
            return;
...
function bearbeiteZeilenZufuegen() {
    ...
    for (zeilenNr = 1; zeilenNr <= lZeilen; zeilenNr += 1) {
        ...
        application.activeWindow.title.endOfField(false); //wichtig bei mehrzeiligen Felder!
        application.activeWindow.title.lineDown(1, false);
        application.activeWindow.title.startOfField(false);
    }
}
